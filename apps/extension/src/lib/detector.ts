...
/**
 * YouTube AI Moderator v5 - 厳格モード検出ロジック
 * 1ヒット即ブロック、しきい値なし
 */

class StrictCommentDetector {
  ...
  detect(text: string, author: string): DetectionResult {
    ...
    // 7. 匂わせ（Nioase/Hints）
    const nioaseResult = this.checkNioase(normalized, skeleton);
    if (nioaseResult.blocked) return nioaseResult;

    // 8. 匂わせ（Hints）- 通常コメントでも対応
    const hintResult = this.checkHints(normalized, skeleton);
    if (hintResult.blocked) return hintResult;

    // 9. 語気強（Strong Tone）- 通常コメントでも対応
  ...
  private checkNioase(normalized: string, _skeleton: string): DetectionResult {
    const hintTerms = [
      'く、くる', 'くるぞ', '来る', '確定', '情報筋', '某', 'らしい', 'っぽい', 
      'みたい', '予想'
    ];
    
    const timeTerms = ['次回', '来週', '明日', '今度', '第', '話'];
    
    for (const hint of hintTerms) {
      if (normalized.includes(hint)) {
        for (const time of timeTerms) {
          if (normalized.includes(time)) {
            return { blocked: true, category: 'nioase', reason: `匂わせ「${hint}」+ 時系列「${time}」` };
          }
        }
      }
    }
...
export const detector = new StrictCommentDetector();